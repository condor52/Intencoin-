...
# Lee el secreto de la variable de entorno BRIDGE_SECRET o usa el valor por defecto.
...
# URL para llamar al API de depósito en tu Wallet (Puerto 5021)
    ...
    if known_wallets:
        for wallet_addr in list(known_wallets):
            try:
                ...
                # Usa 'bridge_secret' para coincidir con la configuración del Wallet (5021)
                payload = {
                    "wallet": wallet_addr,
                    "symbol": "INTEN", 
                    "amount": REWARD_PER_WALLET,
                    "bridge_secret": WALLET_APP_SECRET, # Campo correcto
                }
...
def start_miner_timer():
    ...
    print("---------------------------------------------------------------------")
    print(f"Minero INTENcoin (Puerto 5002) iniciado.")
    print(f"Secreto Cargado: '{WALLET_APP_SECRET}' (Verifica que coincida en 5021)")
...
if __name__ == "__main__":
    ...
    start_miner_timer()
    # CAMBIO CRÍTICO: Ejecutar en el puerto 5002 para evitar conflictos con 5001.
