...
class IntencoinDriver(ChainDriver):
    ...
    def is_configured(self) -> bool:
        return True
    def send(self, from_account: Dict[str, Any], to_address: str, amount: float) -> str:
        # INTEN se mueve en el ledger interno
        return f"INTEN-LEDGER-{uuid.uuid4().hex[:12]}"

class BTCDriver(ChainDriver):
    symbol = "BTC"
    # Por ahora ENABLED para pruebas (sandbox producción)
    def is_configured(self) -> bool:
        return True
    def validate_address(self, address: str) -> bool:
        return address.startswith("1") or address.startswith("3") or address.startswith("bc1")
    def send(self, from_account: Dict[str, Any], to_address: str, amount: float) -> str:
        # Aquí en el futuro integras RPC / API real de BTC
        return f"INTEN-BTC-TX-{uuid.uuid4().hex[:12]}"
...
# External send (from wallet):
POST {{ url_for('api_send_external', _external=True) }} JSON: {"from_wallet":"WALADDR-...","symbol":"XLM","to_address":"G...","amount":1.23}
    </pre>
  </div>
{% endblock %}
"""
...
def send_external():
    ...
    if symbol not in SUPPORTED or amount <= 0:
        return "Invalid request", 400
    if not from_wallet:
        return "Missing from_wallet", 400
    acct = find_account_by_wallet(from_wallet)
    if not acct:
        return "Source wallet not found", 404
    # ensure that the source wallet belongs to the logged-in user's account
    uacct = user_accounts(u['uid'])[0]
    if acct['account_id'] != uacct['account_id']:
        return "Source wallet does not belong to your account", 403
    if acct['balances'][symbol] < amount:
        return "Insufficient funds", 400
    driver = DRIVERS[symbol]
    if not driver.is_configured():
        return "Connector not configured for this chain (no simulated tx)", 400
    acct['balances'][symbol] -= amount
    txid = driver.send(acct, to_address, amount)
    add_ledger({"type":"external","symbol":symbol,"from_account":acct['account_id'],"from_wallet":from_wallet,"to":to_address,"amount":amount,"txid":txid})
    save_state(STATE)
    return redirect(url_for('dashboard'))
...
def api_send_external():
    ...
    amount = float(data.get('amount',0))
    if not from_wallet:
        return jsonify({"ok": False, "error":"missing from_wallet"}), 400
    if symbol not in SUPPORTED or amount <= 0:
        return jsonify({"ok": False, "error":"invalid request"}), 400
    acct = find_account_by_wallet(from_wallet)
    if not acct:
    ...
    uacct = user_accounts(u['uid'])[0]
    if acct['account_id'] != uacct['account_id']:
        return jsonify({"ok": False, "error":"source wallet not yours"}), 403
    if acct['balances'][symbol] < amount:
        return jsonify({"ok": False, "error":"insufficient funds"}), 400
    driver = DRIVERS[symbol]
    if not driver.is_configured():
