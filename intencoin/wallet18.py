...
# Inicializamos MARKET_RATES SOLO para monedas de mercado
MARKET_RATES = {
    "BTC": 0.0,
    "ETH": 0.0,
    "XLM": 0.0,
...
def api_admin_set_rate(symbol):
    if symbol not in SUPPORTED:
        return jsonify({"ok": False, "error": "unsupported symbol"}), 400

    # Soportar JSON (API) y form-data (HTML)
    raw_price = None
    if request.is_json:
...
# =========================================================
# === ENVIAR INTEN A METAMASK COMO DGDM ===================
# =========================================================
...
def api_send_inten_to_metamask_dgdm():
    ...
    tx_hash = mint_dgdm(evm_address, dgdm_amount)
...
# =========================================================
# === ENVIAR INTEN A METAMASK COMO USDT ===================
# =========================================================
...
def api_send_inten_to_metamask_usdt():
    ...
    rates = get_rates()
    usdt_amount = amount_inten * rates["INTEN"] / rates["USDT"]

    amount_wei = web3.to_wei(usdt_amount, 'ether')
    nonce = web3.eth.get_transaction_count(OWNER_ADDRESS)

    tx = usdt_contract.functions.transfer(evm_address, amount_wei).build_transaction({
        'from': OWNER_ADDRESS,
        'nonce': nonce,
        'gas': 200000,
        'gasPrice': web3.to_wei('50', 'gwei')
