...
# Modificado: Líneas ~159-170
def new_user(email: str, password: str, init>
    uid = str(uuid.uuid4())
    # ... (código existente para usuario) ...
    acct_id = "WAL-" + uid.split("-")[0].upp>
    wallet_addresses = []
    for _ in range(max(1, initial_wallets)):
        wallet_addresses.append(gen_wallet_a>
    STATE["accounts"][acct_id] = {
        "account_id": acct_id,
        "owner": uid,
        "balances": {c: 0.0 for c in SUPPORT>
        "wallet_addresses": wallet_addresses,
...
{% block body %}
  ...
  <div class=card>
    ...
    <form method="post" action="{{ url_for('create_wallet') }}">
      <button class=btn type="submit">Create new internal wallet address</button>
    </form>
