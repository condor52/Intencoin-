...
def send_external():
    # 1. Extraer datos primero para revisar si trae el 'secret'
    if request.is_json:
        data = request.get_json()
    else:
    ...
    # Si trae el secret correcto, saltamos require_login()
    secret_sent = data.get("secret")
    u = None
    
    if secret_sent == BRIDGE_SECRET:
        # Creamos un usuario ficticio para que el script continúe sin error
        u = {"uid": "admin-api", "email": "bridge@internal"}
    else:
        # Si no hay secret, intentamos login por sesión normal (Navegador)
        u = require_login()
    ...
    # Validación de pertenencia solo si NO es admin via secret
    if secret_sent != BRIDGE_SECRET:
        uacct = user_accounts(u['uid'])[0]
        if acct['account_id'] != uacct['account_id']:
    ...
    if symbol == "INTEN" and symbol_dest == "BTC":
        ...
        # Cálculo del valor del Target Currency (BTC)
        usd_value = amount_to_debit * rate_inten
        amount_to_send_dest = usd_value / rate_dest 

        # --- CORRECCIÓN CRÍTICA AQUÍ ---
        # Forzamos 8 decimales y convertimos a float para que Bitcoin RPC no dé error -3
        amount_to_send_dest = float("{:.8f}".format(amount_to_send_dest))
        # -------------------------------

        if amount_to_send_dest <= 0:
            return jsonify({"ok": False, "error": "Amount too small for BTC network"}), 400

        # Debitar la Source Currency (INTEN)
        acct["balances"]["INTEN"] -= amount_to_debit
        
        driver = DRIVERS["BTC"]

        try:
            # Ahora amount_to_send_dest es un número limpio (ej: 0.00153061)
            txid = driver.send(acct, to_address, amount_to_send_dest)
        except Exception as e:
            # Si falla el envío, devolvemos el INTEN (opcional pero recomendado)
            acct["balances"]["INTEN"] += amount_to_debit
            return jsonify({"ok": False, "error": str(e)}), 500
