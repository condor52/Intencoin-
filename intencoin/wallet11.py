...
def gen_external_chain_address(symbol: str) -> str:
    """
    Genera una dirección 'dummy' con formato aproximado para cada red.
    Para BTC, usamos Bitcoin Core vía BTCDriver, no esta función.
    """
    s = symbol.upper()
    if s == "ETH":
        # 0x + 40 hex
    ...
    if s == "XLM":
        # Empieza con G, longitud aprox 56
        return "G" + uuid.uuid4().hex.upper().ljust(55, "A")[:55]
    if s == "XRP":
        # Empieza con r, longitud aprox 25–35
        return "r" + uuid.uuid4().hex[:25]
    if s == "LTC":
        # Usamos formato bech32-like
        return "ltc1" + uuid.uuid4().hex[:30]
    # Fallback genérico
    return symbol.upper() + "-" + uuid.uuid4().hex[:16]
...
class LTCDriver(ChainDriver):
    ...
    def send(self, from_account: dict, to_address: str, amount: float) -> str:
        wallet = from_account.get("external_ltc_wallet_id", from_account["account_id"])
        return call_ltc_rpc(
            "sendtoaddress",
...
# Ruta para enviar cripto a ACH a cualquier banco
@app.post("/wallet/ilp/ach-anybank")
async def wallet_to_ach_anybank():
    data = request.get_json()
    wallet = data["wallet"]
    amount = int(data["amount"])
    receiver_bank_spsp = data["receiver_spsp"]
    ...
    result = await ilp_send(amount, receiver_bank_spsp)
    ...
    save_state(STATE)
    return {"ok": True, "ach": result}

# Ruta para enviar cripto a Wire Transfer a cualquier banco
@app.post("/wallet/ilp/wire")
async def wallet_to_wire():
    data = request.get_json()
    wallet = data["wallet"]
    amount = int(data["amount"])
    wire_spsp = data["wire_spsp"]
    ...
    result = await ilp_send(amount, wire_spsp)
...
async def wallet_to_paypal():
    ...
    result = await ilp_send(amount, ILP_PAYPAL_SPSP)
...
def configure_external_wallet():
    ...
    - BTC: usa Bitcoin Core (RPC) vía BTCDriver
    - ETH/XLM/XRP/LTC: genera una dirección dummy local
    """
    u = require_login()
    if not u:
        return redirect(url_for("index"))
    ...
    # Mapear símbolo -> campo en la cuenta
    field_map = {
        "BTC": "external_btc_address",
        "ETH": "external_eth_address",
        "XLM": "external_xlm_address",
    ...
    # Si ya tiene dirección, no hacemos nada
    if acct.get(field_name):
        return redirect(url_for("dashboard"))

    # BTC: usar Bitcoin Core vía BTCDriver
    if symbol == "BTC":
        btc_driver = DRIVERS["BTC"]
        wallet_rpc_name = acct["account_id"]
        ...
        if not new_btc_address or str(new_btc_address).startswith("Error"):
            return "Failed to create BTC wallet", 500
    ...
    # Otras monedas: dirección “dummy”
    new_addr = gen_external_chain_address(symbol)
    acct[field_name] = new_addr
    add_ledger({
        "type": "external_wallet_config",
        "symbol": symbol,
        "account": acct["account_id"],
