...
# ... otras variables globales (como BRIDGE_SECRET)

# Variable global para la URL de notificación del Minero (5001)
MINER_NOTIFY_URL = "http://127.0.0.1:5001/api/notify_deposit"
...
def api_bridge_deposit_new():
    """
    Endpoint de depósito que notifica al Minero (5001) sobre un depósito exitoso
    para disparar la recompensa automática.
    """
    data = request.get_json(force=True)
    secret = data.get("secret")
    ...
    # 3. Respuesta de Fallo de Crédito (Tu lógica original)
    if not acct_id:
        return jsonify({"ok": False, "error":"invalid wallet/symbol/amount"}), 400

    # =======================================================
    # !!! NUEVA LÓGICA DE NOTIFICACIÓN A 5001 !!!
    # =======================================================
    # Solo notificar si el depósito fue exitoso y la cantidad es positiva.
    if acct_id and symbol == "INTEN" and amount > 0:
        try:
            ...
            # Si esto falla, el depósito del cliente ya está en 5020, pero no recibirá recompensa.
            print(f"⚠️ Alerta: Falló la notificación al Blockchain (5001). Error: {e}")

    # =======================================================

    # 4. Respuesta de Éxito (Tu lógica original)
    return jsonify({"ok": True, "credited_account": acct_id, "symbol": symbol, "amount": amount})
