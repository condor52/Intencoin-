...
APP_SECRET = os.environ.get("WALLET_SECRET", "dev-secret-change-me")
...
def call_rpc(method: str, params: list = [], wallet_name: str | None = None) -> Any:
    """Env\u00eda comandos RPC al nodo Bitcoin Core y devuelve el resultado."""
    ...
    try:
        ...
        if data.get("error"):
             ...
             raise RuntimeError(f"BTC RPC Error: {error_message}")
             
        return data["result"] # Bitcoin Core devuelve el resultado en 'result'

    except Exception as e: # <--- Esta es la línea 95
        print(f"RPC Error calling {method} (Wallet: {wallet_name}): {e}")
        return None # Devuelve None en caso de fallo crítico
...
class IntencoinDriver(ChainDriver): # Nivel 0
    symbol = "INTEN" # Nivel 1 (4 espacios)

    def is_configured(self) -> bool:      # Nivel 1 (4 espacios
        # Por ahora siempre configurado; luego aquí puedes chequear tu nodo real
        return True # Nivel 2 (8 espacios)

    def send(self, from_account: Dict[str, Any], to_address: str, amount: float) -> str: # Nivel 1 (4 espacios)
        # envío externo de INTEN
        print(f"[INTEN] Sending {amount} INTEN to {to_address} from {from_account['account_id']}") # Nivel 2 (8 espacios)
        return f"INTEN-TX-{uuid.uuid4().hex[:12]}" # Nivel 2 (8 espacios)



# ... (Clases IntencoinDriver y ChainDriver sin cambios) ...

class BTCDriver(ChainDriver):    symbol = "BTC"
    ...
    # NUEVO: Crear y cargar la billetera RPC para el cliente
    def create_and_get_address(self, wallet_name: str) -> str:
        if not self.is_configured():
            return "Error: Conector BTC no configurado"
            
        try:
            # 1. Intentar crear la billetera (si ya existe, falla con error RPC 4)
            # El tercer parámetro es 'blank=True' para crear sin keys por defecto
            call_rpc("createwallet", [wallet_name, True], wallet_name=None) 
            print(f"[BTC] Creada billetera RPC: {wallet_name}")
        except RuntimeError as e:
            if "Wallet already exists" in str(e):
                # 2. Si ya existe, intentar cargarla
                try:
                    call_rpc("loadwallet", [wallet_name], wallet_name=None)
                    print(f"[BTC] Cargada billetera RPC: {wallet_name}")
                except Exception as load_e:
                    return f"Error al cargar BTC Wallet: {load_e}"
            else:
                return f"Error al crear BTC Wallet: {e}"
        
        # 3. Obtener una nueva dirección de depósito de la billetera recién creada/cargada
        btc_address = call_rpc("getnewaddress", ["", "bech32"], wallet_name=wallet_name)
        return btc_address if btc_address else "Error al obtener dirección"
...
def configure_btc_wallet():
    ...
    # Solo si la direcci\u00f3n no existe
    if not acct.get('external_btc_address'):
        btc_driver = DRIVERS["BTC"]
        
        # El nombre de la billetera RPC ser\u00e1 el ID de la cuenta.
        wallet_rpc_name = acct['account_id']
        
        # Verificar configuraci\u00f3n antes de llamar a RPC
        if not btc_driver.is_configured():
            # En un entorno real, redirigir\u00eda con un error.
            print("ERROR: El conector BTC no est\u00e1 configurado correctamente o RPC ca\u00eddo.")
            return "ERROR: El conector BTC no est\u00e1 configurado. Verifique su nodo RPC.", 500
            
        try:
            # Llama a Bitcoin Core RPC
            ...
            if new_btc_address and not new_btc_address.startswith("Error"):
                ...
                # Guardar el nombre de la billetera RPC para futuros env\u00edos
                ...
                # Si create_and_get_address devuelve un mensaje de error
                print(f"Error al configurar BTC Wallet (RPC Error): {new_btc_address}")
                return f"Error al configurar BTC Wallet (RPC Error): {new_btc_address}", 500
                
        except Exception as e:
            print(f"Excepci\u00f3n cr\u00edtica al configurar BTC Wallet: {e}")
            return f"Excepci\u00f3n al configurar BTC Wallet: {e}", 500
...
def send_external():
    ...
    # Moneda que se DEBITA Y ENVÍA
    symbol = request.form.get('symbol') # e.g., 'INTEN' o 'BTC'
    from_wallet = (request.form.get('from_wallet') or "").strip()
    to_address = (request.form.get('to_address') or "").strip()
    
    try:
        amount_to_debit = float(request.form.get('amount', "0") or 0)
    except:
        amount_to_debit = 0.0

    if symbol not in SUPPORTED or amount_to_debit <= 0:
        return "Invalid request (amount or symbol).", 400
    if not from_wallet or not to_address:
        return "Missing from_wallet or to_address", 400

    acct = find_account_by_wallet(from_wallet)
    if not acct or acct["balances"].get(symbol, 0) < amount_to_debit:
        return "Insufficient funds or source wallet not found", 400

    # Lógica de SWAP (INTEN a OTRA COIN) - CUMPLIENDO TU REGLA
    if symbol == "INTEN" and request.form.get('swap_to_btc'):
        
        # **ESTE ES EL SWAP INTEN -> BTC/ETH/OTRO**
        symbol_dest = request.form.get('swap_to_btc') # Asumimos que el formulario envía esto
        ...
        if rate_inten <= 0 or rate_dest <= 0:
            return f"Exchange rates for INTEN or {symbol_dest} are invalid.", 400

        # Calcular el monto de la moneda de destino
        usd_value = amount_to_debit * rate_inten
        amount_to_send_dest = usd_value / rate_dest
        
        driver = get_driver(symbol_dest)
        if not driver.is_configured():
            return f"Connector not configured for {symbol_dest}", 400
        ...
        # Enviar moneda destino (BTC, ETH, etc.)
        txid = driver.send(acct, to_address, amount_to_send_dest)

        add_ledger({
            "type": "external_swap",
            "spent": "INTEN", "spent_amount": amount_to_debit,
            "sent": symbol_dest, "sent_amount": amount_to_send_dest,
            "from_account": acct["account_id"], "to": to_address, "txid": txid,
        })
        save_state(STATE)
        return redirect(url_for('dashboard'))


    # Lógica de ENVÍO PURO (INTEN a INTEN, BTC a BTC, etc.)
    else:
        # **ESTE ES EL ENVÍO DE INTEN PURO (o de cualquier otra moneda)**
        driver = get_driver(symbol) # El driver es el mismo que el symbol debitado
        
        if not driver.is_configured():
            return f"Connector not configured for {symbol} (Pure Send)", 400
            
        # Debitar la moneda (INTEN en este caso)
        acct["balances"][symbol] -= amount_to_debit
        
        # Enviar (INTEN Driver lo simula, BTC Driver usa RPC)
        txid = driver.send(acct, to_address, amount_to_debit)
        
        add_ledger({
            "type": "external_send_pure",
            "symbol": symbol, "amount": amount_to_debit,
            "from_account": acct["account_id"], "to": to_address, "txid": txid,
        })
        save_state(STATE)
        return redirect(url_for('dashboard'))
...
def api_bridge_deposit_new():
    """
    Endpoint ALTERNO para que un sistema externo deposite fondos.
    Notifica al Minero (5001) cuando un depósito es exitoso para disparar la recompensa.
    """
    # Usando la función auxiliar que ya debe existir: def _credit_by_wallet
    data = request.get_json(force=True)
    secret = data.get("secret")
    ...
    # 2. Crédito usando la función auxiliar
    acct_id = _credit_by_wallet(wallet_addr, symbol, amount)
    ...
    # =======================================================
    # !!! INICIO DE LA NUEVA LÓGICA DE NOTIFICACIÓN A 5001 !!!
    # =======================================================
    if acct_id and symbol == "INTEN" and amount > 0:
        try:
            # Prepara el payload para el puerto 5001
            notify_payload = {
                "wallet": wallet_addr,
                "amount": amount,
                "symbol": symbol,
            ...
            # Llama al puerto 5001 (usando la variable global MINER_NOTIFY_URL)
            # Asegúrate de haber definido: MINER_NOTIFY_URL = "http://127.0.0.1:5001/api/notify_deposit"
            r = requests.post(MINER_NOTIFY_URL, json=notify_payload, timeout=5)
            r.raise_for_status() 
            print(f"✔️ Notificación de depósito de {amount} INTEN enviada a 5001.")
            
        except requests.exceptions.RequestException as e:
            # Si el Minero (5001) no responde, el depósito del cliente es válido, 
            # pero no recibirá la recompensa automática por ahora.
            print(f"⚠️ Alerta: Falló la notificación al Blockchain (5001). Error: {e}")

    # =======================================================
    # !!! FIN DE LA NUEVA LÓGICA !!!
    # =======================================================

    # 4. Respuesta final (original)
    return jsonify({"ok": True, "credited_account": acct_id, "symbol": symbol, "amount": amount})
