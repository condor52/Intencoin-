...
# --- Configuración en tu script de backend ---

# 1. Definición de credenciales
# Estas deben coincidir con tu archivo bitcoin.conf
RPC_USER = "inten" 
...
# Resultado de la URL: "http://inten:intencoin@127.0.0.1:8332"
...
# Funci\u00f3n para realizar llamadas RPC al nodo Bitcoin Core
def call_rpc(method: str, params: list = []) -> Any:
    ...
    try:
        response = requests.post( # ⬅️ Función ahora debe tener el URL, el JSON y la autenticación
            RPC_URL, 
            data=payload, 
            headers=headers,
            auth=HTTPBasicAuth(RPC_USER, RPC_PASSWORD), # ⬅️ Usar las credenciales definidas
            timeout=30 # ⬅️ Buena práctica
        )
        response.raise_for_status() # Lanza error si es 4xx/5xx
        return response.json()
    except Exception as e:
        print(f"RPC Error calling {method}: {e}")
        return {"error": f"Failed RPC call: {e}"}
...
class BTCDriver(ChainDriver):
    ...
    def is_configured(self) -> bool:
        # Solo está “configurado” si tenemos URL + credenciales del nodo
        return bool(BTC_RPC_URL and BTC_RPC_USER and BTC_RPC_PASS)

    def validate_address(self, address: str) -> bool:
        # Para no complicar validación, verificamos que tenga tamaño razonable
        return bool(address and len(address) >= 20)

    def send(self, from_account: Dict[str, Any], to_address: str, amount: float) -> str:
        """
        Envía BTC REAL usando JSON-RPC a Bitcoin Core:
        método: sendtoaddress <address> <amount>
        """
        if not self.is_configured():
            raise RuntimeError("BTC RPC not configured")

        # Construir payload JSON-RPC
        payload = {
            "jsonrpc": "1.0",
            "id": f"inten-{uuid.uuid4().hex[:8]}",
            "method": "sendtoaddress",
            "params": [to_address, float(amount)]
        }
        ...
        if data.get("error"):
            # Bitcoin Core devolvió error (sin fondos, dirección inválida, etc.)
            raise RuntimeError(f"BTC RPC error: {data['error']}")

        txid = data["result"]  # txid real en la red Bitcoin
        print(f"REAL BTC SEND: Sent {amount} BTC to {to_address}, txid={txid}")
        return txid
...
{% block body %}
  ...
  <div class=card>
    ...
    <form method="post" action="{{ url_for('create_wallet') }}">
      <button class=btn type="submit">Create new wallet address</button>
    </form>
    <h3>Balances</h3>
    <table class=table>
  ...
  <div class=row>
    ...
    <form class=card method=post action='{{ url_for('send_external') }}'>
      <h3>Send External (native chain)</h3>
      <p><select name=symbol>{% for s in supported %}<option>{{s}}</option>{% endfor %}</select></p>
      <p><input name=from_wallet placeholder="From wallet address (WALADDR-...)" required></p>
      <p><input name=to_address placeholder="Destination chain address" required></p>
      <p><input name=amount type=number step=any placeholder="Amount" required></p>
      <p><button class=btn>Send</button></p>
      <small>Requires chain connector configured. No simulated tx.</small>
    </form>
  </div>
...
<div id="withdrawal_inten_btc" style="border: 1px solid #ccc; padding: 20px; margin-top: 20px;">
    <h2>1. Retiro y Conversión (Send External)</h2>

    <p style="color: blue;">Su saldo de INTENCOIN: <strong>[AQUÍ_VA_SALDO_INTENCOIN]</strong></p>

    <form id="form_send_external">
        
        <label for="currency_select">Moneda a Retirar:</label>
        <select id="currency_select" name="currency_to_send" required>
            <option value="INTENCOIN" selected>INTENCOIN (Se convierte a BTC)</option>
            <option value="XLM" disabled>XLM (Próximamente)</option>
            <option value="ETH" disabled>ETH (Próximamente)</option>
        </select>
        <br><br>

        <label for="amount_inten">Cantidad de INTENCOIN:</label>
        <input type="number" id="amount_inten" name="amount_inten" min="0.00000001" step="any" required placeholder="Ej: 1000.50">
        <br><br>

        <label for="destination_address">Dirección de Retiro Bitcoin (Externa):</label>
        <input type="text" id="destination_address" name="destination_address" required placeholder="bc1q... o 1... o 3...">
        <br>
        <small style="color: red;">¡Verifique la dirección! Solo direcciones Bitcoin (BTC).</small>
        <br><br>

        <p>BTC Estimados a Enviar: <span id="estimated_btc">0.00000000</span> BTC</p>

        <button type="submit" id="btn_send_external">Enviar INTENCOIN (Convertir y Transferir BTC)</button>
    </form>
...
<div id="internal_wallets_view" style="margin-top: 30px;">
    <h3>2. Sus Direcciones de Depósito Internas</h3>
    <p>Dirección de Depósito INTENCOIN: <input type="text" readonly value="[AQUÍ_VA_ADRE_INTENCOIN]"></p>
    <p>Dirección de Depósito Bitcoin: <input type="text" readonly value="[AQUÍ_VA_ADRE_BITCOIN_CORE]"></p>
</div>


POST /api/send-external JSON: {"from_wallet":"WALADDR-...","symbol":"XLM","to_address":"G...","amount":1.23}
...
def send_external():
    ...
    # Moneda de destino (BTC, ETH, XLM, XRP, LTC)
    symbol_dest = request.form.get('symbol')
    from_wallet = (request.form.get('from_wallet') or "").strip()
    to_address = (request.form.get('to_address') or "").strip()
    # amount = INTEN a gastar
    try:
        inten_amount_spent = float(request.form.get('amount', "0") or 0)
    except:
        inten_amount_spent = 0.0

    if symbol_dest == "INTEN":
        return "Cannot send INTEN to external chain address.", 400
    if symbol_dest not in SUPPORTED or inten_amount_spent <= 0:
    ...
    acct = find_account_by_wallet(from_wallet)
    if not acct:
        return "Source wallet not found", 404
