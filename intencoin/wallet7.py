...
APP_SECRET = os.environ.get("WALLET_SECRET", "BRIDGE_SECRET")
...
def call_rpc(method: str, params: list = [], wallet_name: str | None = None) -> Any:
    ...
    try:
        ...
        if data.get("error"):
             ...
             # *** CORRECCIÓN MÁS ROBUSTA PARA loadwallet (Código -4 o mensaje) ***
             is_loadwallet_error = (method == "loadwallet" and 
                                    (error_code == -4 or "already loaded" in error_message.lower()))
             
             if is_loadwallet_error:
                 print(f"[RPC] '{wallet_name}' ya estaba cargada o existe (Código {error_code}). Continuando.")
                 return {"name": wallet_name} 
             
             # Si no es un error de "ya cargada", lanzamos el error RPC
...
class BTCDriver(ChainDriver):
    ...
    def create_and_get_address(self, wallet_name: str) -> str:
        ...
        # --- Parte 1: Intentar crear o manejar el error de existencia ---
        try:
            # 1. Intentar crear la billetera (parámetro 'blank=True')
            call_rpc("createwallet", [wallet_name, True], wallet_name=None) 
            print(f"[BTC] Creada billetera RPC: {wallet_name}. (Procediendo a cargar...)")
        
        except RuntimeError as e:
            # Capturar errores que indican que el archivo ya existe o está cargado
            error_msg = str(e)
            if "exists" in error_msg or "loaded" in error_msg: 
                print(f"[BTC] Wallet ya existe o está cargada en disco.")
            else:
                # Si es un error desconocido (ej: permisos, disco lleno, etc.)
                return f"Error al crear BTC Wallet: {e}"
        
        # --- Parte 2: Forzar la carga (esto resuelve el error 500) ---
        try:
            # 2. Forzar la carga, sin importar si la creación falló (porque existía) o tuvo éxito.
            call_rpc("loadwallet", [wallet_name], wallet_name=None)
            print(f"[BTC] Carga de billetera RPC confirmada: {wallet_name}")
        except Exception as load_e:
             # Si no se puede cargar, es un error irrecuperable.
             return f"Error al cargar BTC Wallet: {load_e}"


        # --- Parte 3: Obtener la dirección ---
        try:
            # 3. Obtener una nueva dirección de depósito de la billetera recién cargada.
...
def send_external():
    ...
    # Lógica de SWAP INTEN -> BTC (o cualquier otra cripto)
    if symbol == "INTEN" and request.form.get('swap_to_btc'):
        symbol_dest = request.form.get('swap_to_btc')
        ...
        # Envío directo sin swap
        driver = get_driver(symbol)
        if not driver.is_configured():
            return f"Connector not configured for {symbol}", 400
