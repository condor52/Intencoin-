...
def configure_external_wallet():
   ...
   # =========================
    # MONEDAS NO-BTC (LTC, ETH, XLM, XRP)
    # =========================
    if symbol in ("LTC", "ETH", "XLM", "XRP"):
        dummy_addr = f"{symbol}-ADDR-{uuid.uuid4().hex[:24].upper()}"
        acct[field_name] = dummy_addr
    ...
    # 🔴 SEGURIDAD FINAL (NUNCA DEBE LLEGAR AQUÍ)
    return redirect(url_for("dashboard"))
