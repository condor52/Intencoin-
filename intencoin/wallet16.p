...
SUPPORTED = ["INTEN", "DGDM", "BTC", "ETH", "XLM", "XRP", "LTC"]
DEFAULT_RATES = {
    "INTEN": 0.50,
    "BTC":   65000.0,
    "ETH":   3000.0,
...
# =========================================================
# === CONFIGURACIÓN DE TOKENS EVM =========================
# =========================================================

EVM_TOKENS = {
    "DGDM": {
        "rpc": "https://1rpc.io/matic",
        "contract": "0xEe9cf06834d3FD930811EdF021cd391493C749A8",
        "decimals": 18
    },
    "USDT": {
        "rpc": "https://1rpc.io/matic",
...
# --- MARKET PRICE UPDATER (CoinGecko) ---
def update_market_rates():
    ...
    try:
        ...
        # Actualizamos los MARKET_RATES con los valores obtenidos
        MARKET_RATES["BTC"] = float(data["bitcoin"]["usd"])
        MARKET_RATES["ETH"] = float(data["ethereum"]["usd"])
        MARKET_RATES["XLM"] = float(data["stellar"]["usd"])
        MARKET_RATES["XRP"] = float(data["ripple"]["usd"])
        MARKET_RATES["LTC"] = float(data["litecoin"]["usd"])

        # Guardamos estos valores en el estado
        for coin in MARKET_RATES:
            STATE["rates"][coin] = MARKET_RATES[coin]
...
DGDM_CONTRACT = "0xEe9cf06834d3FD930811EdF021cd391493C749A8"
OWNER_PRIVATE_KEY = os.getenv("PRIVATE_KEY")  # llave privada 0x3335...
OWNER_ADDRESS = "0x3335bddbaa5a9343b1883896102c6e5bfd621e7b"
...
def new_user(email: str, password: str, initial_wallets: int = 1) -> str:
    ...
    STATE["accounts"][acct_id] = {
        ...
        # Campos para wallets externas
        "external_btc_address": None,
        "external_btc_wallet_id": None,
        "external_eth_address": None,
...
def mint_dgdm(to_address: str, amount: float):
    amount_wei = web3.to_wei(amount, 'ether')
    nonce = web3.eth.get_transaction_count(OWNER_ADDRESS)

    tx = dgdm.functions.mint(to_address, amount_wei).build_transaction({
        'from': OWNER_ADDRESS,
        'nonce': nonce,
        'gas': 300000,
...
def swap_inten_to_dgdm(acct, amount_inten):
    # 1. Debitar INTEN del ledger interno
    acct["balances"]["INTEN"] -= amount_inten

    # 2. Convertir INTEN → DGDM usando tus tasas
    rates = get_rates()
    dgdm_amount = amount_inten * rates["INTEN"] / rates["DGDM"]

    # 3. Mint DGDM al usuario
    user_wallet = acct["external_eth_address"]
    tx = mint_dgdm(user_wallet, dgdm_amount)
...
def swap_dgdm_to_inten(acct, dgdm_amount):
    # 1. Burn DGDM
    user_wallet = acct["external_eth_address"]
    tx = burn_dgdm(user_wallet, dgdm_amount)

    # 2. Convertir DGDM → INTEN
    rates = get_rates()
    inten_amount = dgdm_amount * rates["DGDM"] / rates["INTEN"]

    # 3. Acreditar INTEN
    acct["balances"]["INTEN"] += inten_amount

    # 4. Registrar en ledger
    add_ledger({
        "type": "swap_dgdm_to_inten",
        "to": acct["account_id"],
        "amount_inten": inten_amount,
        "amount_dgdm": dgdm_amount,
        "tx": tx
    })
...
SUPPORTED = ["INTEN", "USDT", "USDC", "DAI", "WETH", "WMATIC"]

DEFAULT_RATES = {
    "INTEN": 0.50, # 1 INTEN = 0.50 USD (ejemplo)
    "USDT": 1.00,
    "USDC": 1.00,
    "DAI": 1.00,
    "WETH": 3000.0, # ejemplo
...
<table class="table">
  ...
  <tr>
    ...
    <td>
      {% if not acct[field] %}
        <form method="post" action="{{ url_for('configure_external_wallet') }}">
...
<form class=card method=post action='{{ url_for('swap_inten_to_token') }}'>
  <h3>Swap INTEN → Token (interno)</h3>
  <p><b>From:</b> INTEN (internal balance)</p>
  <p>
    <b>To Token:</b>
  ...
  <p><input name=from_wallet placeholder="Your internal wallet (WALADDR-...)" required></p>
  <p><input name=amount type=number step=any placeholder="Amount of INTEN to swap" required></p>
  <p><button class=btn>Swap INTEN → Token</button></p>
  <small>Uses sovereign rates to convert INTEN to selected token inside this system.</small>
</form>



<form class=card method=post action='{{ url_for('send_token_to_polygon') }}'>
  <h3>Send Token → Polygon (Metamask)</h3>
  <p>
    <b>Token:</b>
    <select name=token_symbol>
      <option>USDT</option>
  ...
  <p><input name=from_wallet placeholder="Your internal wallet (WALADDR-...)" required></p>
  <p><input name=to_address placeholder="Destination Metamask address (0x...)" required></p>
  <p><input name=amount type=number step=any placeholder="Amount of token to send" required></p>
  <p><button class=btn>Send Token to Polygon</button></p>
  <small>Debits token balance here and instructs bank + smart contract to deliver on Polygon.</small>
</form>
...
@app.route("/send-token-to-polygon", methods=["POST"])
def send_token_to_polygon():
    user = require_login()
    if not user:
        return redirect(url_for("index"))

    from_wallet = request.form.get("from_wallet", "").strip()
    ...
    if acct["owner"] != user["uid"]:
        abort(403, "Not your wallet")

    balances = acct.get("balances", {})
    if balances.get(token_symbol, 0.0) < amount_token:
        abort(400, "Insufficient token balance")

    # Debitar token interno
    balances[token_symbol] -= amount_token

    add_ledger({
        "type": "send_token_polygon",
    ...
    # Notificar al banco + contrato inteligente
    try:
        payload = {
            ...
            "metamask_funds_wallet": METAMASK_FUNDS_WALLET,
            "bank_escrow_id": BANK_ESCROW_ID,
        }
        r = requests.post(f"{BANK_API_URL}/api/escrow/send-token-polygon",
                          json=payload, timeout=15)
        r.raise_for_status()
        bank_resp = r.json()
    except Exception as e:
        print(f"[BANK] Error calling send-token-polygon: {e}")
        bank_resp = {"ok": False, "error": str(e)}

    return jsonify({
        "ok": True,
        "message": "Token debited internally and Polygon transfer requested.",
        "bank_response": bank_resp,
    })




@app.route("/swap-inten-to-token", methods=["POST"])
def swap_inten_to_token():
    user = require_login()
    if not user:
        return redirect(url_for("index"))
    ...
    rates = get_rates()
    if token_symbol not in rates:
        abort(400, "Unsupported token")

    balances = acct.get("balances", {})
    if balances.get("INTEN", 0.0) < amount_inten:
        abort(400, "Insufficient INTEN balance")

    usd_value = amount_inten * rates["INTEN"]
    token_rate = rates[token_symbol] # 1 TOKEN = X USD
    token_amount = usd_value / token_rate

    # Debitar INTEN, acreditar token
    balances["INTEN"] -= amount_inten
    balances[token_symbol] = balances.get(token_symbol, 0.0) + token_amount
